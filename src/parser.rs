...
pub enum NodeKind {
    ...
    IfStmt {
        ...
        then_body: Vec<SyntaxNode>,
        else_body: Vec<SyntaxNode>,
    },
    WhileStmt {
        condition: Box<SyntaxNode>,
        body: Vec<SyntaxNode>,
    ...
    // simple statements
    Assignment {
        target: Vec<SyntaxNode>,
        value: Box<SyntaxNode>,
    },
    Return {
        retval: Box<SyntaxNode>,
    },
...
impl From<Pair<'_, Rule>> for SyntaxNode {
    fn from(pair: Pair<Rule>) -> Self {
        ...
        match rule {
            ...
            | Rule::simple_stmt
            | Rule::compound_stmt
            | Rule::primary => inner.next().unwrap().into(),

            Rule::program | Rule::do_end => {
                let body = inner.next_body();
                SyntaxNode::new(&pair, NodeKind::Sequence { body })
            }
            ...
            Rule::while_stmt => {
                let condition = Box::new(inner.next().unwrap().into());
                let body = inner.next_body();
                SyntaxNode::new(&pair, NodeKind::WhileStmt { condition, body })
            }
            Rule::for_stmt => {
            ...
            Rule::array => {
                let elements = pair.clone().into_inner().map(SyntaxNode::from).collect();
                SyntaxNode::new(&pair, NodeKind::ArrayLiteral { elements })
            }
            Rule::dict => {
                let entries = pair
                    .clone()
                    ...
                    .collect();
                SyntaxNode::new(&pair, NodeKind::DictLiteral { entries })
            }
            Rule::func_def_one_line => {
                let (name, params) = collect_func_sig(&mut inner);
                let body = vec![inner.next().unwrap().into()];
                SyntaxNode::new(&pair, NodeKind::FuncDef { name, params, body })
            }
            Rule::func_def => {
                let (name, params) = collect_func_sig(&mut inner);
                let body = inner.next_body();
            ...
            Rule::return_stmt => {
                ...
                SyntaxNode::new(&pair, NodeKind::Return { retval })
            }
            Rule::assignment => {
                let [target, value] = inner.next_chunk().unwrap();
                let target = target.into_inner().map(SyntaxNode::from).collect();
                let value = Box::new(value.into());
                SyntaxNode::new(&pair, NodeKind::Assignment { target, value })
            }
            Rule::access => {
                let path = inner.map(SyntaxNode::from).collect();
                SyntaxNode::new(&pair, NodeKind::Access { path })
            }
            Rule::comparison | Rule::bool_expr | Rule::term | Rule::factor => {
            ...
            Rule::call => {
                ...
                SyntaxNode::new(&pair, NodeKind::Call { target, args })
            }
            Rule::number => {
                let value = pair.as_str().parse().unwrap();
                SyntaxNode::new(&pair, NodeKind::NumberLiteral { value })
            }
            Rule::string => {
                let lexeme = pair.as_str();
                let value = lexeme[1..lexeme.len() - 1].to_string(); // remove quotes
                SyntaxNode::new(&pair, NodeKind::StringLiteral { value })
            }
            Rule::boolean => {
                ...
                SyntaxNode::new(&pair, NodeKind::BooleanLiteral { value })
            }
            Rule::ident => SyntaxNode::new(
                &pair,
                NodeKind::Ident {
                    name: pair.as_str().to_string(),
                },
            ),
            Rule::nil => SyntaxNode::new(&pair, NodeKind::NilLiteral),
